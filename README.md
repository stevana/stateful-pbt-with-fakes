...
database and the fake filesystem, while the stateful and parallel
property-based tests of the filesystem alone do the contract tests that
...
implemented in about 400 lines of code, which is comparible to the 300
...
1.  Can we make the thread scheduling in the parallel testing
...
2.  When there's a mismatch between the fake and the real system in the
    ...
    for each command;
3.  In our examples we've mostly used a uniform distribution of
    ...
    - [*Beginner’s Luck: A Language for Property-Based
      ...
      2017).
4.  We can also imagine not only using randomness for generating
...
5.  Stateful and parallel testing together with reusing the fakes to
...
6.  The more operations that are concurrent at the same time the more
    ...
    same state. We can expoit this fact to check less histories;
7.  The properties we've covered in this post are so called safety
    properties, i.e. "something bad will not happen". There's another
    class of properties called liveness and fariness properties, i.e.
...
8.  We've discussed how we can hide for example the file system behind
