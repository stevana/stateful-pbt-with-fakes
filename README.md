...
To my best knowledge, as of July 2024, the following table summarises
...
The last new thing to note here is that `WhereIs_` returns the thread id
...
With fakes we can get fast and deterministic integration tests by
...
We can then use the fake file system when we integration test and thus
get fast and deterministic tests, and then use the real file system when
we deploy.
...
Note that `prog` is just a silly example, in a real system the component
...
necessary to fake[^8] so to finish of let's sketch how the same
...
experimenting in their favorite programming languages[^10]. Feel free to
get [in touch](https://stevana.github.io/about.html) or open an
[issue](https://github.com/stevana/stateful-pbt-with-fakes) in case
there's anything I can help with. Who knows, together, we might even be
able to slightly improve the state of property-based testing libraries!
...
- Anthony Lloyd
  ...
  about the happy state of property-based testing in C# in general, and
  about some innovative functionality in CsCheck in particular;
- Pierre "ibizaman" Penninckx
  ...
  to one of his own earlier posts about stateful property-based testing
  in Go.
...
[^2]: There's some room for error here from the users side, e.g. the
    user could create non-unique references. In a proper library one
...
[^4]: Another idea might be to drop all preconditions in the parallel
    case and make all commands be able to fail gracefully instead of
    crashing, e.g. `Write_ (Either DoesntExist ())`.

    The problem with this approach is that some examples, such as the
    [ticket
    dispenser](https://github.com/stevana/stateful-pbt-with-fakes/blob/main/src/Example/TicketDispenser.hs),
    have initialisation commands such as `New` which create a ticket
    ...
    generating: `Fork [New, New]`, which doesn't make sense. It should
    also be noted that making `New` fail gracefully when a `New` has
    already been executed would need a global boolean flag, which is
    ugly.
...
[^6]: The sequential variant of the process registry example first
...
[^7]: I believe the post-condition formulation is more general, as it
    allows a relational rather than a functional specification. So I
    don't think we can show the converse of what Edsko did, i.e.
    implement a post-condition style specification on top of our
    fake-based one.

[^8]: Unless we want to test what happens when failures, such as the
    disk being full etc.
...
[^10]: In the unlikely case that there's someone out there who wants to
    dig even deeper into this topic, then I've compiled a list of
