...
We can also improve the shrinking by moving commands outside of forks, e.g.
`[Fork [a, b]] ==> [Fork [a], Fork [b]]`, thus making the program more
sequential, and moving forks after smaller forks, e.g. `[Fork [a, b], Fork [c]]
...
1. Can we make the thread scheduling in the parallel testing deterministic like
   in the
   [*PULSE*](https://www.cse.chalmers.se/~nicsma/papers/finding-race-conditions.pdf)
...
3. In our examples we've mostly used a uniform distribution of commands, for our
...
4. We can also imagine not only using randomness for generating commands, see
...
5. Stateful and parallel testing together with reusing the fakes to test bigger
...
6. The more operations that are concurrent at the same time the more work the
   ...
   histories;
7. The properties we've covered in this post are so called safety properties,
...
8. We've discussed how we can hide for example the file system behind an
